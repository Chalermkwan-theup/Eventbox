-- 0004_fix_reserve_tickets_ambiguous_status.sql
-- BUG FIX (found by running against a real Postgres — static review missed it).
--
-- reserve_tickets() declares `RETURNS TABLE (... status order_status ...)`, so `status`
-- is an OUT variable in scope for the whole body. The event-availability check used an
-- UNqualified column reference:  `select 1 from event where ... and status = 'published'`.
-- Postgres could not tell whether `status` meant the OUT variable or event.status and
-- raised SQLSTATE 42702 "column reference \"status\" is ambiguous" — on EVERY call, so
-- the entire reservation flow was broken.
--
-- Fix: add `#variable_conflict use_column` (bare identifiers resolve to table columns;
-- all OUT params are only ever produced by the final qualified RETURN QUERY, so this is
-- safe) and qualify the event reference as `e.status`.
...
revoke execute on function reserve_tickets(uuid, jsonb, text) from public, anon;
